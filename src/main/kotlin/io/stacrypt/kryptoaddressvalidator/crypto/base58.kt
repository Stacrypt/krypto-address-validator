...
fun String.decodeBase58WithChecksum(): ByteArray {
    /**
     * We do not now why the original org.komputing.kbase58.decodeBase58WithChecksum not working.
     * This code repeated in respect to the error of tron addresses base58 checksum errors.
     */
    val rawBytes = decodeBase58()
    if (rawBytes.size < 4) {
        throw Exception("Too short for checksum: $this l:  ${rawBytes.size}")
    }
    val checksum = rawBytes.takeLast(4).toByteArray()
    val computedChecksum =
        this.decodeBase58().dropLast(4).toByteArray().sha256DigestDual().take(4).toByteArray()
    if (checksum.toNoPrefixHexString() == computedChecksum.toNoPrefixHexString()) {
        return rawBytes.dropLast(4).toByteArray()
    } else {
        throw IllegalArgumentException(
            "Checksum mismatch: ${checksum.toNoPrefixHexString()} is not computed" +
                    " checksum ${computedChecksum.toNoPrefixHexString()}"
        )
    }
